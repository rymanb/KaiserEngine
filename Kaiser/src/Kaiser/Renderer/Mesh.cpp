...
namespace Kaiser {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			KS_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		KS_CORE_INFO("Loading mesh: {0}", filename.c_str());

		Assimp::Importer importer;

		const aiScene* scene = importer.ReadFile(filename, ImportFlags);
		if (!scene || !scene->HasMeshes())
			KS_CORE_ERROR("Failed to load mesh file: {0}", filename);

		aiMesh* mesh = scene->mMeshes[0];

		KS_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
		KS_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
		...
		{
			KS_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
